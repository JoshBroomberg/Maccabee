...
def generate_transformed_covariate_space(covariate_symbols, subfunction_form_probabilities):
    ...
    for subfunction_form_name, subfunction_form in Constants.SUBFUNCTION_FORMS.items():
        ...
        covariate_combinations = np.array(
            list(combinations(covariate_symbols, len(subfunc_covariate_symbols))))
        ...
        subfunctions.extend([subfunc_expression.subs(
...
def generate_aligned_treatment_and_outcome_covariates(covariate_symbols):
    ...
    true_covariates = np.array(
        list(set(true_treatment_covariates + true_outcome_covariates)))
    ...
    true_outcome_covariates = list(
        shared_confounders.union(true_outcome_covariates))
    true_treatment_covariates = list(
        shared_confounders.union(true_treatment_covariates))
...
def generate_treatment_function(treatment_subfunctions, data):
    ...
    initialized_subfunctions = initialize_expression_constants(
        treatment_subfunctions)
...
def created_simulated_data_from_random_covariates():
    ...
    true_outcome_covariates, true_treatment_covariates = \
        generate_aligned_treatment_and_outcome_covariates(
            potential_confounder_symbols)

    ############################

    treatment_function = generate_treatment_function(
        true_treatment_covariates, data)

    ###########################

    outcome_function = generate_outcome_function(
        true_outcome_covariates, data)

    ###########################

    simulated_data = simulate_treatment_and_potential_outcomes(
        treatment_function, outcome_function, data)
